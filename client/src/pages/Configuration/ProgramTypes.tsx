...
const ProgramTypes: React.FC = () => {
  ...
  const fetchProgramTypes = useCallback(async () => {
    try {
      setLoading(true);
      const response = await configAPI.programTypes.getAll({
      ...
      setMessage({ type: 'error', text: 'Failed to fetch program types' });
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    fetchProgramTypes();
  }, [pagination.currentPage, searchTerm, fetchProgramTypes]);
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.program_name.trim()) {
      ...
      hasErrors = true;
    } else if (formData.program_name.trim().length < 2) {
      errors.program_name = 'Program name must be at least 2 characters long';
      hasErrors = true;
    } else if (formData.program_name.trim().length > 50) {
      errors.program_name = 'Program name must not exceed 50 characters';
      hasErrors = true;
    }
  ...
  const handleEdit = (type: ProgramType) => {
    setFormData({
      program_name: type.program_name,
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this program type?')) {
      try {
        await configAPI.programTypes.delete(id.toString());
        setMessage({ type: 'success', text: 'Program type deleted successfully!' });
        fetchProgramTypes();
      } catch (error: any) {
        setMessage({ 
          type: 'error', 
          text: error.response?.data?.message || 'Failed to delete program type' 
        });
      }
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await configAPI.programTypes.update(editingId.toString(), formData);
        setMessage({ type: 'success', text: 'Program type updated successfully!' });
      } else {
        await configAPI.programTypes.create(formData);
        setMessage({ type: 'success', text: 'Program type created successfully!' });
      }
      ...
      setMessage({ 
        type: 'error', 
        text: error.response?.data?.message || 'Failed to save program type' 
      });
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="grid-page">
          ...
          <div className="modal">
            <div className="modal-header">
              <div className="modal-title">
                <h2>{editingId ? 'Edit Program Type' : 'Add new Program Type'}</h2>
                <p>Enter program type details below</p>
              </div>
              <button
                type="button"
            ...
            <div className="modal-content">
              <form id="program-type-form" onSubmit={handleSubmit} className="business-form">
                ...
                <div className="business-form-section">
                  ...
                  <div className="business-form-group business-form-group-full">
                    <label htmlFor="program_name" className="business-form-label">
                    ...
                    <div className="business-form-input-wrapper">
                      <input
                        ...
                        id="program_name"
                        name="program_name"
                        className={`business-form-input ${validationErrors.program_name ? 'is-invalid' : ''}`}
                        value={formData.program_name}
                        onChange={handleInputChange}
                        placeholder="e.g., Undergraduate (UG), Postgraduate (PG)"
                        required
                      />
                      ...
                      {validationErrors.program_name && (
                        <div className="business-form-error">{validationErrors.program_name}</div>
                      )}
                    </div>
                  <div className="business-form-group business-form-group-full">
                    ...
                    <div className="business-form-textarea-wrapper">
                      <textarea
                        ...
                        onChange={handleInputChange}
                        rows={3}
                        placeholder="Brief description of the program type"
                      />
      ...
      <div className="page-header">
        <h1>Program Types</h1>
        <p>Manage program type information and details</p>
      </div>
      ...
      <div className="grid-container">
        ...
        {loading ? (
          ...
          <div className="grid-empty">
            <div className="grid-empty-icon">📚</div>
            <p>No program types found</p>
            <p>Get started by adding your first program type.</p>
          </div>
        ) : (
            ...
            <table className="grid-table">
              ...
              <tbody>
                {programTypes.map((type) => (
                  <tr key={type.id}>
                    <td>
                      <div style={{ fontWeight: '600', color: '#1f2937' }}>{type.program_name}</div>
                    </td>
                    <td>
                      <div style={{ color: '#6b7280' }}>
                        {type.description ? truncateText(type.description, 100) : 'No description'}
                      </div>
                    </td>
        ...
        <div className="grid-footer">
          {pagination.totalPages > 1 ? (
            ...
            <div>Total: {programTypes.length} items</div>
          )}
        </div>
      </div>
    </div>
