...
const Weightage: React.FC = () => {
  const [weightages, setWeightages] = useState<WeightageData[]>([]);
  ...
  const fetchWeightages = useCallback(async () => {
    try {
      setLoading(true);
      const response = await configAPI.weightage.getAll({
      ...
      if (response.data.data) {
        setWeightages(response.data.data);
        setPagination(response.data.pagination);
      }
    } catch (error) {
      setMessage({ type: 'error', text: 'Failed to fetch weightages' });
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    fetchWeightages();
  }, [pagination.currentPage, searchTerm, fetchWeightages]);
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.level_name.trim()) {
      ...
      errors.level_name = 'Level name must be at least 2 characters long';
      hasErrors = true;
    } else if (formData.level_name.trim().length > 50) {
      errors.level_name = 'Level name must not exceed 50 characters';
      hasErrors = true;
    }
    ...
    if (!formData.acronym.trim()) {
      ...
      hasErrors = true;
    } else if (formData.acronym.trim().length !== 1) {
      errors.acronym = 'Acronym must be exactly one letter';
      hasErrors = true;
    }

    if (formData.percentage_weightage < 0) {
      errors.percentage_weightage = 'Percentage weightage cannot be negative';
      hasErrors = true;
    } else if (formData.percentage_weightage > 100) {
      errors.percentage_weightage = 'Percentage weightage cannot exceed 100';
      hasErrors = true;
    }
  ...
  const handleEdit = (weightage: WeightageData) => {
    setFormData({
      level_name: weightage.level_name,
      acronym: weightage.acronym,
      status: weightage.status,
      percentage_weightage: weightage.percentage_weightage,
      description: weightage.description || ''
    });
    setEditingId(weightage.id || null);
    setValidationErrors({});
    setMessage(null);
    setShowAccordion(true);
  };
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this weightage?')) {
      try {
        await configAPI.weightage.delete(id.toString());
        setMessage({ type: 'success', text: 'Weightage deleted successfully!' });
        fetchWeightages();
      } catch (error: any) {
        setMessage({ 
          type: 'error', 
          text: error.response?.data?.message || 'Failed to delete weightage' 
        });
      }
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        const response = await configAPI.weightage.update(editingId.toString(), formData);
        setMessage({ type: 'success', text: 'Weightage updated successfully!' });
      } else {
        const response = await configAPI.weightage.create(formData);
        setMessage({ type: 'success', text: 'Weightage created successfully!' });
      }
      ...
      setMessage({ 
        type: 'error', 
        text: error.response?.data?.message || 'Failed to save weightage' 
      });
    } finally {
      setSaving(false);
    }
  ...
  const getWeightageColor = (percentage: number) => {
  ...
  return (
    <div className="grid-page">
          ...
          <div className="modal">
            <div className="modal-header">
              <div className="modal-title">
                <h2>{editingId ? 'Edit Weightage Level' : 'Add new Weightage Level'}</h2>
                <p>Enter weightage level details below</p>
              </div>
              <button
                type="button"
            ...
            <div className="modal-content">
              <form id="weightage-form" onSubmit={handleSubmit} className="business-form">
                ...
                <div className="business-form-section">
                  ...
                  <div className="business-form-grid">
                    ...
                    <div className="business-form-group">
                      <label htmlFor="percentage_weightage" className="business-form-label">
                        Percentage Weightage <span className="required">*</span>
                      </label>
                      <div className="business-form-input-wrapper">
                        <input
                          type="number"
                          ...
                          className={`business-form-input ${validationErrors.percentage_weightage ? 'is-invalid' : ''}`}
                          value={formData.percentage_weightage}
                          onChange={handleInputChange}
                          required
                          min="0"
                          max="100"
                      ...
                      {validationErrors.percentage_weightage && (
                        <div className="business-form-error">{validationErrors.percentage_weightage}</div>
                      )}
                    </div>
                    <div className="business-form-group">
                      <label htmlFor="status" className="business-form-label">
                      ...
                      <div className="business-form-input-wrapper">
                        <select
                          id="status"
                          name="status"
                          className={`business-form-input ${validationErrors.status ? 'is-invalid' : ''}`}
                      ...
                      {validationErrors.status && (
                        <div className="business-form-error">{validationErrors.status}</div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="business-form-group business-form-group-full">
                    ...
                    <div className="business-form-textarea-wrapper">
                      <textarea
                        ...
                        onChange={handleInputChange}
                        rows={3}
                        placeholder="Brief description of this weightage level..."
                      />
      ...
      <div className="page-header">
        <h1>Weightage Levels</h1>
        <p>Manage weightage levels and their percentage values</p>
      </div>
      ...
      <div className="grid-container">
          ...
          <div className="grid-header-actions">
            ...
            <div className="grid-search">
              ...
              <form onSubmit={handleSearch}>
                <input
                  type="text"
                  placeholder="Search Weightage Levels..."
                  value={searchTerm}
        ...
        {loading ? (
          <div className="grid-loading">
            ...
            <p>Loading weightage levels...</p>
          </div>
        ) : weightages.length === 0 ? (
          <div className="grid-empty">
            <div className="grid-empty-icon">⚖️</div>
            <p>No weightage levels found</p>
            <p>Get started by adding your first weightage level.</p>
          </div>
        ) : (
            ...
            <table className="grid-table">
              ...
              <tbody>
                {weightages.map((weightage) => (
                  <tr key={weightage.id}>
                    <td>
                      <div style={{ fontWeight: '600', color: '#1f2937' }}>{weightage.level_name}</div>
                    </td>
                    <td>
                      <span style={{ 
                        background: '#6b7280', 
                    ...
                    <td>
                      <span style={{ 
                        background: getWeightageColor(weightage.percentage_weightage), 
                        color: 'white', 
                        padding: '4px 8px', 
                        borderRadius: '4px', 
                        fontSize: '12px',
                        ...
                        {weightage.percentage_weightage}%
                      </span>
                    </td>
                    <td>
                      <span style={{ 
                        background: getStatusColor(weightage.status), 
                        color: 'white', 
                        padding: '4px 8px', 
                        borderRadius: '4px', 
                        fontSize: '12px',
                        ...
                        {weightage.status.charAt(0).toUpperCase() + weightage.status.slice(1)}
                      </span>
                    </td>
                    <td>
                      <div style={{ color: '#6b7280' }}>
                        {weightage.description ? truncateText(weightage.description, 80) : '-'}
                      </div>
                    </td>
                    <td className="grid-actions">
                      <button
                        className="grid-action-btn edit"
                        onClick={() => handleEdit(weightage)}
                        title="Edit"
                      >
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
        ...
        <div className="grid-footer">
          {pagination.totalPages > 1 ? (
            ...
            <div>Total: {weightages.length} items</div>
          )}
        </div>
      </div>
    </div>
