...
const BOSMembers: React.FC = () => {
  ...
  const [facultyTypes, setFacultyTypes] = useState<{id: number, type_name: string}[]>([]);
  ...
  const gridColumns = [
    { key: 'display_name', title: 'Name', width: '30%' },
    { key: 'email', title: 'Email', width: '30%' },
    { key: 'faculty_type', title: 'Type', width: '20%' },
  ...
  // Fetch BOS members
  const fetchBOSMembers = async (page = 1, search = '') => {
    setLoading(true);
    try {
      const response = await configAPI.bosMembers.getAll({ page, search });
      setBOSMembers(response.data.data);
      setPagination(response.data.pagination);
    } catch (error) {
      setMessage({ type: 'error', text: 'Failed to fetch BOS members' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchFacultyTypes = async () => {
    try {
      const response = await configAPI.facultyTypes.getAll();
      setFacultyTypes(response.data.data || []);
    } catch (error) {
      console.error('Failed to fetch faculty types:', error);
      setFacultyTypes([]); // Ensure it's always an array
    }
  };
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.faculty_type.trim()) {
      errors.faculty_type = 'Faculty type is required';
      hasErrors = true;
    }
    ...
    if (!formData.email.trim()) {
      ...
      const isEmailChanged = editingId ? 
        bosMembers.find(m => m.id === editingId)?.email !== formData.email : true;
      
      if (isEmailChanged && bosMembers.some(m => m.email.toLowerCase() === formData.email.toLowerCase())) {
    ...
    if (!formData.bos_department_id) {
      errors.bos_department_id = 'BOS Department is required';
      hasErrors = true;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const submitData = {
        ...
        bos_department_id: parseInt(formData.bos_department_id),
        is_active: formData.is_active
      };

      if (editingId) {
        await configAPI.bosMembers.update(editingId.toString(), submitData);
        setMessage({ type: 'success', text: 'BOS Member updated successfully!' });
      } else {
        await configAPI.bosMembers.create(submitData);
        setMessage({ type: 'success', text: 'BOS Member created successfully!' });
      }

      await fetchBOSMembers(pagination.currentPage, searchTerm);
      ...
      let errorMessage = 'Failed to save BOS member';
  ...
  // Handle edit
  const handleEdit = (member: BOSMember) => {
    setFormData({
      faculty_type: member.faculty_type,
      title: member.title || '',
      ...
      bos_department_id: member.bos_department_id ? member.bos_department_id.toString() : '',
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this BOS member?')) {
      try {
        await configAPI.bosMembers.delete(id.toString());
        setMessage({ type: 'success', text: 'BOS Member deleted successfully!' });
        fetchBOSMembers(pagination.currentPage, searchTerm);
      } catch (error) {
        setMessage({ type: 'error', text: 'Failed to delete BOS member' });
      }
    }
  };
  ...
  const handleGridEdit = (member: BOSMember) => {
    handleEdit(member);
  };

  const handleGridDelete = (member: BOSMember) => {
    if (member.id) {
  ...
  const handlePageChange = (page: number) => {
    fetchBOSMembers(page, searchTerm);
  };

  // Prepare data for Grid component
  const gridData = bosMembers.map(member => ({
    ...member,
  ...
  return (
    <div className="grid-page">
      ...
      <div className="page-header">
        <h1>BOS Members</h1>
        <p>Manage Board of Studies (BOS) members and their details.</p>
      </div>
          ...
          <div className="modal">
            <div className="modal-header">
              <div className="modal-title">
                <h2>{editingId ? 'Edit BOS Member' : 'Add New BOS Member'}</h2>
                <p>Enter BOS member details below</p>
              </div>
              <button
                type="button"
            ...
            <div className="modal-content">
              <form id="bos-member-form" onSubmit={handleSubmit} className="business-form">
                {/* Basic Information Section */}
                <div className="business-form-section">
                  <h3 className="business-form-section-title">Basic information</h3>
                  <div className="business-form-grid">
                    <div className="business-form-group">
                      <label htmlFor="faculty_type" className="business-form-label">
                      ...
                      <div className="business-form-input-wrapper">
                        ...
                        >
                          ...
                          {Array.isArray(facultyTypes) && facultyTypes.map((type) => (
                    ...
                    <div className="business-form-group">
                      <label htmlFor="title" className="business-form-label">Title</label>
                      <div className="business-form-input-wrapper">
                        <select
                          id="title"
                          name="title"
                ...
                <div className="business-form-section">
                  ...
                  <div className="business-form-grid">
                    <div className="business-form-group">
                      <label htmlFor="designation" className="business-form-label">
                    ...
                    <div className="business-form-group">
                      <label htmlFor="bos_department_id" className="business-form-label">
                        BOS Department <span className="required">*</span>
                      </label>
                      <div className="business-form-input-wrapper">
                        <select
                          id="bos_department_id"
                          name="bos_department_id"
                          className="business-form-input"
                          value={formData.bos_department_id}
                          onChange={handleInputChange}
                          required
                        >
                          <option value="">Select BOS department</option>
                ...
                <div className="business-form-section">
                  ...
                  <div className="business-form-grid">
                    ...
                    <div className="business-form-group">
                      <label htmlFor="power" className="business-form-label">Power</label>
                      <div className="business-form-input-wrapper">
                        <input
                          type="text"
                          id="power"
                          ...
                          onChange={handleInputChange}
                          placeholder="Enter power/authority"
                        />
                        <div className="business-form-input-icon">
                          <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
      ...
      <Grid
        ...
        searchable={true}
        loading={loading}
        addButtonText="Add BOS Member"
        searchPlaceholder="Search BOS Members"
      />
